...
impl DecodeFormat {
    pub fn from_content_type(content_type: Option<&str>) -> Self {
        ...
        match ct.as_deref() {
            Some("application/json") | Some("application/otlp+json") => DecodeFormat::Json,
            Some("application/x-protobuf")
            | Some("application/protobuf")
            | Some("application/otlp") => DecodeFormat::Protobuf,
            _ => DecodeFormat::Auto,
        }
    }
}

pub fn looks_like_json(body: &Bytes) -> bool {
    body.iter()
...
pub fn finite_float_to_vrl(value: f64, context: &str) -> VrlValue {
    if value.is_nan() || value.is_infinite() {
        tracing::debug!(
            value = %value,
            %context,
            "converting non-finite float to null during decode"
        );
        VrlValue::Null
    } else {
        // Use unwrap_or_else for defensive handling of edge cases
        // (e.g., denormals that might slip through is_nan/is_infinite checks)
        match NotNan::new(value) {
            ...
            Err(_) => {
                tracing::warn!(
                    value = %value,
                    %context,
                    "unexpected non-finite float after validation, converting to null"
                );
                VrlValue::Null
            }
        }
...
pub fn otlp_any_value_to_vrl(av: &AnyValue) -> VrlValue {
    match av.value.as_ref() {
        ...
        Some(any_value::Value::DoubleValue(d)) => finite_float_to_vrl(*d, "protobuf decode"),
...
// ============================================================================
// JSON utilities - shared between logs_json.rs, traces_json.rs, and HEC decoder
// ============================================================================

/// Convert raw serde_json::Value to VRL Value.
/// Used by HEC decoder and any other JSON-based decoders.
/// Uses finite_float_to_vrl for consistent NaN/Infinity handling.
pub fn serde_json_to_vrl(json: serde_json::Value) -> VrlValue {
    match json {
        ...
        serde_json::Value::Number(n) => {
            if let Some(i) = n.as_i64() {
                ...
                finite_float_to_vrl(f, "serde_json decode")
            } else {
                VrlValue::Null
            }
        }
        serde_json::Value::String(s) => VrlValue::Bytes(Bytes::from(s)),
        serde_json::Value::Array(arr) => {
            VrlValue::Array(arr.into_iter().map(serde_json_to_vrl).collect())
        }
...
impl JsonNumberOrString {
    ...
    pub fn as_raw_string(&self) -> Option<String> {
        match self {
            JsonNumberOrString::String(s) => Some(s.clone()),
            JsonNumberOrString::Number(n) => Some(n.to_string()),
            JsonNumberOrString::Missing => None,
        }
    }
}
...
pub fn json_any_value_to_vrl(av: JsonAnyValue) -> VrlValue {
    if let Some(s) = av.string_value {
        ...
        i.as_i64()
            .map(VrlValue::Integer)
            .or_else(|| {
                i.as_raw_string()
                    .map(|raw| VrlValue::Bytes(Bytes::from(raw)))
            })
            .unwrap_or(VrlValue::Null)
    } else if let Some(d) = av.double_value {
        finite_float_to_vrl(d, "json decode")
...
// ============================================================================
// Protobuf utilities - shared between logs.rs and traces_proto.rs
// ============================================================================
...
/// Traverse OTLP resources and scopes, reusing resource/scope VRL values.
