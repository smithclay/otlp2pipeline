...
pub fn execute_plan(args: PlanArgs) -> Result<()> {
    ...
    eprintln!();
    eprintln!("  Lambda:");
    eprintln!("    - {}-ingest (ARM64, {}MB)", stack_name, 512);
    eprintln!("    - Function URL (auth: NONE)");
    eprintln!("    - CodeFetcher (downloads from GitHub releases)");
    eprintln!();
    eprintln!("  IAM:");
    eprintln!("    - LambdaExecutionRole (Firehose write)");
    eprintln!("    - CodeFetcherRole (S3 upload)");
    eprintln!("    - FirehoseRole (S3 Tables write)");
    eprintln!();
    eprintln!("  Monitoring:");
    eprintln!("    - CloudWatch alarm (lambda-errors)");
    eprintln!();
    eprintln!("  Logging:");
