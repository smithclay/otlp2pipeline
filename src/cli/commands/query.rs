...
fn bucket_name(env_name: &str) -> String {
    format!("frostbit-{}", env_name.replace('_', "-"))
}
...
pub async fn execute_query(args: QueryArgs) -> Result<()> {
    ...
    let init_sql = format!(
        r#"-- DuckDB init for frostbit environment: {}
-- Auto-generated by frostbit CLI
...
"#,
    ...
    // Write to temp file
    let init_file =
        std::env::temp_dir().join(format!("frostbit-duckdb-init-{}.sql", std::process::id()));
